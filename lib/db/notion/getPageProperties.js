...
export default async function getPageProperties(
  ...
  properties.type = Array.isArray(properties.type) ? (properties.type[0] || 'Post') : (properties.type || 'Post')
  properties.status = Array.isArray(properties.status) ? (properties.status[0] || 'Published') : (properties.status || 'Published')
  properties.category = Array.isArray(properties.category) ? (properties.category[0] || null) : (properties.category || null)
  properties.comment = Array.isArray(properties.comment) ? (properties.comment[0] || null) : (properties.comment || null)
