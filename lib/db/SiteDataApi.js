...
export async function resolvePostProps({ prefix, slug, suffix, locale, from }) {
  ...
  const ensureBlockMap = async (post) => {
    ...
    try {
      ...
      console.log('[DEBUG] post.blockMap keys:', Object.keys(post.blockMap))
      console.log('[DEBUG] post.blockMap.collection_query:', JSON.stringify(post.blockMap.collection_query || {}))
    } catch (e) {
  ...
  if (props.post && props.post.type === undefined) {
