  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'absolute', width: "100%" }}>
        ...
        <i className='far fa-copyright' /> {`${copyrightDate}`} <a href={siteConfig('LINK')} className='text-xs pt-4 text-light-400 dark:text-gray-400'>{siteConfig('AUTHOR')}</a>. All Rights Reserved. 
        <div style={{ position: 'relative', width: "100%", zIndex: 100 }}>
        <p className='text-xs pt-2 text-light-500 dark:text-gray-500'><i className='mx-1 fas fa-code' />Powered by <a href='https://github.com/siuze/NotionNext' className='dark:text-gray-300'>NotionNext {siteConfig('VERSION')}</a>.</p>
        <p className='text-xs pt-2 text-light-500 dark:text-gray-500'><i className='mx-1 fas fa-bolt' />Acceleration with <a href='https://blogCDN.net' className='dark:text-gray-300'>blogCDN </a>.</p>
      </div>
      </div>
      <Fish />
    </footer>
